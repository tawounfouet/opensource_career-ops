...
Fuld opsætningsguide: [docs/getting-started/setup.md](docs/getting-started/setup.md).
...
Som standard stoler `node scripts/js/scan.mjs` (`npm run scan`) på det, hvert ATS-feed returnerer. Nogle virksomheder lader forældede opslag blive stående, selv efter rekrutteringen er lukket. Brug `--verify` for at køre Playwright efter API-fasen og frasortere udløbne stillinger, før de tilføjes pipelinen:

```bash
node scripts/js/scan.mjs --verify          # token-fri søgning + liveness-verifikation via Playwright
```
