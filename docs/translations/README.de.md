...
Siehe [docs/getting-started/setup.md](docs/getting-started/setup.md) für die vollständige Setup-Anleitung und [docs/getting-started/budget.md](docs/getting-started/budget.md) für günstige Nutzung mit eigenen oder lokalen Modellen.
...
**Durchsuchte Jobbörsen:** 21 Provider-Module decken ATS-APIs, boardweite Feeds, XML/RSS-Feeds, Markdown-Feeds und lokale Parser ab. Siehe [Supported job boards](docs/guides/supported-job-boards.md) für die vollständige Tabelle.

Standardmäßig vertraut `node scripts/js/scan.mjs` (alias `npm run scan`) den Rückgaben der ATS-Feeds. Einige Unternehmen lassen alte Stellenanzeigen öffentlich verfügbar, obwohl Rollen bereits geschlossen sind. Mit `--verify` startet Playwright nach dem API-Lauf und entfernt abgelaufene Stellenanzeigen, bevor sie in `pipeline.md` landen:
