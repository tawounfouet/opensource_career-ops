...
Pełny przewodnik po konfiguracji: [docs/getting-started/setup.md](docs/getting-started/setup.md).
...
Domyślnie `node scripts/js/scan.mjs` (`npm run scan`) ufa temu, co zwraca każdy feed ATS. Niektóre firmy zostawiają nieaktualne ogłoszenia nawet po zamknięciu rekrutacji. Przekaż `--verify`, żeby uruchomić Playwright po fazie API i odfiltrować wygasłe oferty przed dodaniem do pipeline'u:

```bash
node scripts/js/scan.mjs --verify          # zero-tokenowe wyszukiwanie + weryfikacja liveness przez Playwright
```
